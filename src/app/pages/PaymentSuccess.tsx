...
const PENDING_CAFE24_CHARGE_STORAGE_KEY = "pending_cafe24_charge";
const PENDING_TOSS_CHARGE_STORAGE_KEY = "pending_toss_charge";
...
export default function PaymentSuccess() {
  ...
  const storedPendingCharge = (() => {
    try {
      const raw = localStorage.getItem(PENDING_CAFE24_CHARGE_STORAGE_KEY);
  ...
  useEffect(() => {
    if (paymentKey && tossOrderId && tossAmount) {
      ...
      const confirmTossPayment = async () => {
        try {
          const pendingRaw = localStorage.getItem(PENDING_TOSS_CHARGE_STORAGE_KEY);
          const pending = pendingRaw ? JSON.parse(pendingRaw) : null;
          const response = await fetch(
            `https://${projectId}.supabase.co/functions/v1/make-server-53dba95c/payment/confirm`,
            {
              method: "POST",
              headers: {
          ...
          if (cancelled) return;
          setStatus("credited");
          localStorage.removeItem(PENDING_TOSS_CHARGE_STORAGE_KEY);
          ...
          console.error("Toss payment confirm error:", confirmError);
          if (!cancelled) {
    ...
    const run = async () => {
      try {
        ...
        if (nextStatus === "credited") {
          localStorage.removeItem(PENDING_CAFE24_CHARGE_STORAGE_KEY);
          const loggedIn = isLoggedIn || (await checkLoginStatus());
          if (loggedIn) {
            await refreshUserData();
          }
